...
function checkNewsworthiness(request) {
  ...
  // ── FILTER 1: RELEVANCE — Does it serve the student public? ──
  ...
  const isUniversityWide = universityWide.some((k) =>
    clientType.includes(k) || text.includes(k)
  );
  const isDepartmentLevel = departmentLevel.some((k) =>
    clientType.includes(k) || text.includes(k)
  );
  ...
  if (isLowRelevance) {
    ...
    // Default: student org or unclassified — still counts
    filtersPassed++;
    reasons.push("Organization-level relevance — serves a recognized campus community.");
  }

  // ── FILTER 2: NEWSWORTHINESS TYPE — Is it worth publishing? ──
  ...
  if (isCritical) {
    ...
    // Neutral — still give partial credit
    filtersPassed += 0.5;
    reasons.push("Moderate story potential — outcome depends on event significance.");
  }

  // ── FILTER 3: RESOURCE JUSTIFICATION — Worth sending staffers? ──
  ...
  // ── Score mapping (0–3 filters) → (1–5 score) ──
  let score;
