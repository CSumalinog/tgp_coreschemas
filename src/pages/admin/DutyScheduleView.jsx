...
import {
  Box, Typography, CircularProgress, Alert, Avatar, useTheme,
} from "@mui/material";
...
const DAY_CFG = [
  { bg: "#eff6ff", color: "#1d4ed8", dot: "#3b82f6", darkBg: "rgba(59,130,246,0.08)",  darkColor: "#93c5fd" },
  { bg: "#f5f3ff", color: "#6d28d9", dot: "#8b5cf6", darkBg: "rgba(139,92,246,0.08)", darkColor: "#c4b5fd" },
  { bg: "#f0fdf4", color: "#15803d", dot: "#22c55e", darkBg: "rgba(34,197,94,0.08)",  darkColor: "#86efac" },
  { bg: "#fff7ed", color: "#c2410c", dot: "#f97316", darkBg: "rgba(249,115,22,0.08)", darkColor: "#fdba74" },
  { bg: "#fdf2f8", color: "#9d174d", dot: "#ec4899", darkBg: "rgba(236,72,153,0.08)", darkColor: "#f9a8d4" },
];
...
export default function DutyScheduleView() {
  ...
  const loadSchedules = useCallback(async () => {
    ...
    const { data, error: fetchErr } = await supabase
      .from("duty_schedules")
      .select(`id, duty_day, created_at, staffer:staffer_id ( id, full_name, section, role, division, avatar_url )`)
  ...
  const dayCounts = DAY_LABELS.map((_, i) => schedules.filter((s) => s.duty_day === i).length);
  ...
  const columns = [
      ...
      renderCell: (p) => {
        ...
        return (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, height: "100%" }}>
            <Avatar
              src={url}
              sx={{ width: 28, height: 28, fontSize: "0.62rem", fontWeight: 700, backgroundColor: GOLD, color: CHARCOAL, flexShrink: 0 }}
            >
    ...
    },
    {
      ...
      renderCell: (p) => {
        const cfg = SECTION_CFG[p.value] || { bg: "#f9fafb", color: "#6b7280", dot: "#9ca3af" };
        return (
          <Box sx={{ display: "flex", alignItems: "center", height: "100%" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, px: 1.25, py: 0.35, borderRadius: "6px", backgroundColor: isDark ? `${cfg.dot}15` : cfg.bg }}>
      ...
      renderCell: (p) => (
        <Box sx={{ display: "flex", alignItems: "center", height: "100%" }}>
          <Box sx={{
            ...
            <Typography sx={{ fontFamily: dm, fontSize: "0.68rem", fontWeight: 500, color: "text.secondary", textTransform: "capitalize" }}>
              {p.value || "—"}
            </Typography>
          </Box>
        </Box>
    ...
    },
    {
      ...
      renderCell: (p) => {
        ...
        return (
          <Box sx={{ display: "flex", alignItems: "center", height: "100%" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, px: 1.25, py: 0.35, borderRadius: "6px", backgroundColor: isDark ? cfg.darkBg : cfg.bg }}>
              <Box sx={{ width: 5, height: 5, borderRadius: "50%", backgroundColor: cfg.dot, flexShrink: 0 }} />
              <Typography sx={{ fontFamily: dm, fontSize: "0.68rem", fontWeight: 600, color: isDark ? cfg.darkColor : cfg.color, letterSpacing: "0.04em" }}>
                {DAY_LABELS[p.value] ?? "—"}
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: "background.default", minHeight: "100%", fontFamily: dm }}>
      ...
      {!activeSemester && !loading && (
        <Alert severity="info" sx={{ mb: 3, borderRadius: "8px", fontFamily: dm, fontSize: "0.78rem" }}>
        ...
        <Box sx={{ display: "flex", gap: 1, mb: 3, flexWrap: "wrap" }}>
          {DAY_LABELS.map((day, i) => {
            ...
            return (
              ...
              >
                <Typography sx={{ fontFamily: dm, fontSize: "0.65rem", fontWeight: 700, color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.08em", mb: 0.5 }}>
                  {day}
                </Typography>
                <Box sx={{ display: "flex", alignItems: "baseline", gap: 0.4, mb: 1 }}>
                  <Typography sx={{ fontFamily: dm, fontSize: "1.35rem", fontWeight: 800, letterSpacing: "-0.03em", color: isDark ? cfg.darkColor : cfg.color, lineHeight: 1 }}>
                    {dayCounts[i]}
                  </Typography>
                  <Typography sx={{ fontFamily: dm, fontSize: "0.7rem", color: "text.secondary" }}>/10</Typography>
                </Box>
                {/* Progress bar */}
                <Box sx={{ height: 3, borderRadius: "2px", backgroundColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(53,53,53,0.06)", overflow: "hidden" }}>
                  <Box sx={{
      ...
      {activeSemester && (
        <Box sx={{ mb: 2, borderBottom: `1px solid ${border}`, display: "flex", gap: 0 }}>
          {["All", ...COVERAGE_SECTIONS].map((sec) => {
            ...
            const count    = sec === "All" ? schedules.length : schedules.filter((s) => s.staffer?.section === sec).length;
            return (
              <Box
                key={sec}
      ...
      <Box sx={{ width: "100%", overflowX: "auto" }}>
        <Box sx={{ minWidth: 600, bgcolor: "background.paper", borderRadius: "10px", border: `1px solid ${border}`, overflow: "hidden" }}>
          {loading ? (
