...
// ── Blocked Dates Panel ───────────────────────────────────────────────────────
function BlockedDatesPanel({ events, loading, isDark, onEdit, onDelete, onAdd, collapsed, onToggle }) {
  ...
  if (collapsed) {
    return (
      <Box sx={{
        ...
        {upcoming.length > 0 && (
          <Tooltip title={`${upcoming.length} upcoming blocked date${upcoming.length > 1 ? "s" : ""}`} placement="left" arrow>
            <Box sx={{
              width: 36, height: 36, borderRadius: "10px",
              border: `1px solid ${border}`,
              backgroundColor: "background.paper",
              ...
              <BlockOutlinedIcon sx={{ fontSize: 12, color: "text.disabled" }} />
              <Typography sx={{ fontFamily: dm, fontSize: "0.6rem", fontWeight: 700, color: "text.secondary", lineHeight: 1 }}>
                {upcoming.length}
              </Typography>
            </Box>
          </Tooltip>
...
export default function CalendarManagement() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const { data, error: fetchErr } = await supabase
        .from("blocked_dates")
        .select("id, title, notes, start_date, end_date, start_time, end_time, blocked_by, created_at")
      ...
      setEvents((data || []).map((row) => {
        ...
        const startDate = row.start_time
          ...
          : new Date(`${row.start_date}T${isMulti ? "00:00:00" : "08:00:00"}`);
  ...
  const handleSaveEvent = async (eventData) => {
    ...
    try {
      ...
      const startTime = isMulti ? null : `${String(start.getHours()).padStart(2,"0")}:${String(start.getMinutes()).padStart(2,"0")}:00`;
      const endTime   = isMulti ? null : `${String(end.getHours()).padStart(2,"0")}:${String(end.getMinutes()).padStart(2,"0")}:00`;
  ...
  const renderAllDayBlock = (ev) => (
    ...
    >
      <Box sx={{
        ...
        <Typography sx={{ fontFamily: dm, fontSize: "0.58rem", opacity: 0.75, lineHeight: 1.3 }}>
          All day
        </Typography>
      </Box>
    </Box>
    ...
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 3 }, backgroundColor: "background.default", minHeight: "100%", fontFamily: dm }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Box sx={{ textAlign: "center" }}>
          <Typography sx={{ fontFamily: dm, fontWeight: 700, fontSize: { xs: "0.88rem", sm: "1rem" }, color: "text.primary", letterSpacing: "-0.02em" }}>
            Week {weekNumber} of {currentMonthYear}
          </Typography>
          <Typography sx={{ fontFamily: dm, fontSize: "0.7rem", color: "text.secondary", mt: 0.2, display: { xs: "none", sm: "block" } }}>
                    ...
                    <Box key={date.toISOString()} sx={{
                      ...
                      {dayBlocks.length > 0 && (
                        <Box sx={{ display: "flex", justifyContent: "center", mt: 0.4 }}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 0.4, px: 0.75, py: 0.2, borderRadius: "4px", backgroundColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(53,53,53,0.06)" }}>
                            <Box sx={{ width: 4, height: 4, borderRadius: "50%", backgroundColor: isDark ? "rgba(255,255,255,0.35)" : "rgba(53,53,53,0.35)" }} />
                            <Typography sx={{ fontFamily: dm, fontSize: "0.58rem", color: "text.secondary", fontWeight: 600 }}>
                              {dayBlocks.length}
                            </Typography>
                          </Box>
                        </Box>
                  ...
                  return (
                    <React.Fragment key={hour}>
                      <Box sx={{
                        ...
                        <Typography sx={{ fontFamily: dm, fontSize: "0.62rem", color: "text.disabled", userSelect: "none", lineHeight: 1, whiteSpace: "nowrap" }}>
                      ...
                      {weekDays.map((date) => {
                        ...
                        return (
                          <Box key={iso + hour} position="relative" minHeight={SLOT_HEIGHT} sx={{
                            ...
                            backgroundColor: allDayEvs.length > 0 ? "transparent" : densityBg,
