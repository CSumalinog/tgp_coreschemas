// src/pages/admin/NotificationCleanup.jsx
...
export default function NotificationCleanup() {
  ...
  const fetchNotifStats = useCallback(async () => {
    ...
    const { count: read } = await supabase.from("notifications").select("id", { count: "exact", head: true }).eq("is_read", true);
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, fontFamily: dm }}>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography sx={{ fontFamily: dm, fontSize: "0.72rem", color: "text.secondary", mt: 0.3 }}>Manage and clean up system notifications.</Typography>
      </Box>
            ...
            <Tooltip title={notifStats.read > 0 ? `Clear ${notifStats.read} read notification(s)` : "No read notifications to clear"} arrow>
              <span>
                <IconButton
                  ...
                  onClick={() => openConfirm("Clear Read Notifications", `Delete ${notifStats.read} read notification(s) across all users? This cannot be undone.`, purgeReadNotifications, true)}
                  sx={{
                    borderRadius: "4px",
                    p: 0.6,
