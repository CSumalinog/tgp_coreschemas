// src/pages/admin/ArchiveManagement.jsx
...
export default function ArchiveManagement({ embedded = false }) {
  ...
  const restoreFromArchive = useCallback(async (ids) => {
    setActionLoading(true);
    try {
      const { error } = await supabase.from("coverage_requests").update({ archived_at: null }).in("id", ids);
      if (error) throw error;
        ...
        return (
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} onClick={() => setAnchorEl(null)} transformOrigin={{ horizontal: "right", vertical: "top" }} anchorOrigin={{ horizontal: "right", vertical: "bottom" }} slotProps={{ paper: { sx: { minWidth: 160, borderRadius: "10px", fontFamily: dm, mt: 0.5, boxShadow: isDark ? "0 8px 24px rgba(0,0,0,0.4)" : "0 8px 24px rgba(0,0,0,0.08)" } } }}>
              ...
              <MenuItem onClick={() => openConfirm("Move to Trash", `Move "${row.title}" to trash? You can unarchive it later.`, () => moveToTrash([row.id]))} sx={{ fontFamily: dm, fontSize: "0.78rem", gap: 1, py: 0.75, color: RED }}>
    ...
    <Box sx={{ p: embedded ? 0 : { xs: 2, sm: 3 }, fontFamily: dm }}>
      {!embedded && (
        <Box sx={{ mb: 3 }}>
          ...
          <Typography sx={{ fontFamily: dm, fontSize: "0.72rem", color: "text.secondary", mt: 0.3 }}>Archive completed, declined, or cancelled requests.</Typography>
        </Box>
      )}
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
        ...
        {archivableRequests.length > 0 && (
          <Card>
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
              ...
              <Box
                onClick={!actionLoading ? () => openConfirm("Archive All", `Archive all ${archivableRequests.length} completed/declined/cancelled request(s)?`, () => archiveRequests(archivableRequests.map((r) => r.id))) : undefined}
            ...
            <Typography sx={{ fontFamily: dm, fontSize: "0.72rem", color: "text.disabled", mb: 1.5 }}>
              {archivableRequests.length} request(s) with Completed/Declined/Cancelled status ready to be archived.
            </Typography>
        ...
        {/* Archived Requests */}
        <Card>
          <SectionLabel icon={ArchiveOutlinedIcon} label="Archived Requests" />
          {archivedRequests.length === 0 ? (
            <Box sx={{ py: 6, display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
              ...
              <Typography sx={{ fontFamily: dm, fontSize: "0.82rem", color: "text.disabled" }}>No archived requests</Typography>
            </Box>
          ) : (
            <>
              <BulkBar
