...
const URGENCY = {
  light: {
    ...
    critical: { color:RED,      border:"rgba(198,40,40,0.3)",  dot:RED,    label:"Critical" },
    soon:     { color:"#8a6800",border:"rgba(245,197,43,0.5)", dot:GOLD,   label:"Soon"     },
  ...
  dark: {
    ...
    soon:     { color:GOLD,      border:"rgba(245,197,43,0.35)", dot:GOLD,      label:"Soon"     },
    upcoming: { color:"#555",    border:"#3a3a3a",               dot:"#444",    label:"Upcoming" },
  },
};
...
const PIPELINE_STAGES = [
  ...
  { key:"On Going",     label:"On Going",     icon:RadioButtonCheckedOutlinedIcon,     navTab:"Approved",    isPhase2:true },
  { key:"Completed",    label:"Completed",    icon:TaskAltOutlinedIcon,                navTab:"Approved",    isPhase2:true },
];
...
export default function Dashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const onGoing  =(requests||[]).filter((r)=>r.status==="On Going").length;
      const completed=(requests||[]).filter((r)=>r.status==="Completed").length;
      const covered  =approved+onGoing+completed;
      const assigned =(requests||[]).filter((r)=>["Assigned","For Approval","Approved","On Going","Completed"].includes(r.status)).length;
  ...
  const goToRequest=(id,s)=>{
    const m={Pending:"Pending",Forwarded:"Forwarded",Assigned:"Forwarded","For Approval":"For Approval",Approved:"Approved","On Going":"Approved",Completed:"Approved",Declined:"Declined"};
    navigate("/admin/request-management",{state:{tab:m[s]||"all",openRequestId:id}});
  };
  ...
  const kpiCards=[
    {label:"Total",     value:perfStats.total,    sub:"requests submitted",                                    navTab:"all",     isRed:false, icon:AssignmentOutlinedIcon        },
    {label:"Approved",  value:perfStats.approved, sub:`${perfStats.total>0?((perfStats.approved/perfStats.total)*100).toFixed(0):0}% of total`, navTab:"Approved",isRed:false,icon:CheckCircleOutlineOutlinedIcon},
    {label:"Declined",  value:perfStats.declined, sub:`${perfStats.declineRate}% decline rate`,                navTab:"Declined",isRed:true,  icon:CancelOutlinedIcon            },
    {label:"Completion",value:`${perfStats.completionRate}%`,sub:"assigned → covered",                         navTab:null,      isRed:false, icon:TrendingUpOutlinedIcon        },
  ];
  ...
  const cardHeaderSx={
    ...
    backgroundColor: isDark?"rgba(255,255,255,0.015)":"rgba(53,53,53,0.018)",
  };
  ...
  return (
    <Box sx={{p:{xs:1.5,sm:2,md:2.5},backgroundColor:"background.default",minHeight:"100%",fontFamily:dm}}>
      ...
      ════════════════════════════════════════════ */}
      <Box sx={{
        mb:2, borderRadius:"12px", overflow:"hidden", position:"relative",
        background: isDark
        ...
        <Box sx={{position:"absolute",bottom:-30,left:60,width:120,height:120,borderRadius:"50%",backgroundColor:"rgba(245,197,43,0.03)",pointerEvents:"none"}}/>

        <Box sx={{px:{xs:2.5,md:3},py:{xs:2,md:2.5},display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:2,position:"relative"}}>

          {/* Left — status + quick counts */}
          <Box sx={{display:"flex",alignItems:"center",gap:{xs:2,md:3.5},flexWrap:"wrap"}}>
            <Box>
              <Box sx={{display:"flex",alignItems:"center",gap:0.75,mb:0.5}}>
                <Box sx={{
                  width:5,height:5,borderRadius:"50%",backgroundColor:GOLD,
                  boxShadow:`0 0 7px ${GOLD}`,
                  animation:"blink 2s ease-in-out infinite",
                  "@keyframes blink":{"0%,100%":{opacity:1},"50%":{opacity:0.25}},
                }}/>
                <Typography sx={{fontFamily:dm,fontSize:"0.6rem",fontWeight:700,color:GOLD,textTransform:"uppercase",letterSpacing:"0.13em"}}>
                  Live
                </Typography>
              </Box>
              {forApprovalCount>0?(
                <Typography sx={{fontFamily:dm,fontSize:{xs:"0.92rem",md:"1rem"},fontWeight:800,color:"#fff",lineHeight:1.25,letterSpacing:"-0.02em"}}>
              ...
              <Typography sx={{fontFamily:dm,fontSize:"0.7rem",color:"rgba(255,255,255,0.35)",mt:0.35}}>
                {isAllTime?"All-time data":selectedSemester?.name||"—"}
              </Typography>
            </Box>

            <Box sx={{width:"1px",height:38,backgroundColor:"rgba(255,255,255,0.07)",display:{xs:"none",sm:"block"}}}/>

            {/* Quick pulse numbers */}
            <Box sx={{display:"flex",gap:{xs:2,md:3},flexWrap:"wrap"}}>
              {[
                {label:"Pending",     value:statusCounts["Pending"]||0,      activeColor:GOLD       },
                {label:"For Approval",value:statusCounts["For Approval"]||0, activeColor:"#60a5fa"  },
                {label:"Overdue",     value:overdueCount,                     activeColor:"#f87171"  },
              ].map((s)=>(
                <Box key={s.label}>
                  <Typography sx={{fontFamily:dm,fontSize:"1.25rem",fontWeight:800,letterSpacing:"-0.03em",lineHeight:1,color:s.value>0?s.activeColor:"rgba(255,255,255,0.15)"}}>
                    {s.value}
                  </Typography>
                  <Typography sx={{fontFamily:dm,fontSize:"0.6rem",color:"rgba(255,255,255,0.28)",mt:0.2,textTransform:"uppercase",letterSpacing:"0.07em"}}>
                    {s.label}
                  </Typography>
                </Box>
              ))}
          ...
          <Box sx={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:0.75}}>
            {[
              ...
              <Box key={btn.label} onClick={btn.onClick} sx={{
                display:"flex",alignItems:"center",gap:0.5,
                px:1.25,py:0.55,borderRadius:"7px",cursor:"pointer",
            ...
            <FormControl size="small" disabled={isAllTime}>
              <Select
                ...
                sx={{
                  fontFamily:dm,fontSize:"0.73rem",borderRadius:"7px",
      ...
      {loading?(
        ...
        <Box sx={{display:"flex",flexDirection:"column",gap:1.5}}>
            ...
            {kpiCards.map((k)=>{
              ...
              return (
                <Box
                  ...
                  sx={{
                    bgcolor:"background.paper",borderRadius:"10px",border:`1px solid ${border}`,
                    px:2,py:1.5,
                    display:"flex",alignItems:"center",gap:1.5,
                    ...(k.navTab?{cursor:"pointer",transition:"border-color 0.15s, box-shadow 0.15s","&:hover":{borderColor:GOLD,boxShadow:`0 2px 10px ${GOLD_12}`}}:{}),
                  }}
                >
                  <Box sx={{
                    width:38,height:38,borderRadius:"9px",flexShrink:0,
                    display:"flex",alignItems:"center",justifyContent:"center",
                    backgroundColor:k.isRed?(isDark?RED_15:RED_L):(isDark?GOLD_12:"rgba(245,197,43,0.09)"),
                  }}>
                    <Icon sx={{fontSize:17,color:k.isRed?RED:GOLD}}/>
                  </Box>
                  <Box>
                    <Typography sx={{fontFamily:dm,fontSize:"1.55rem",fontWeight:800,lineHeight:1,letterSpacing:"-0.03em",color:k.isRed?RED:"text.primary"}}>
                      {k.value}
                    </Typography>
                    <Typography sx={{fontFamily:dm,fontSize:"0.67rem",color:"text.disabled",mt:0.2}}>
                      {k.label} · {k.sub}
                    </Typography>
                  </Box>
                </Box>
          ...
          {/* ════════════════════════════════════════════
              MAIN ROW — Attention (dominant) + Right col
          ════════════════════════════════════════════ */}
          <Box sx={{display:"grid",gridTemplateColumns:{xs:"1fr",lg:"1fr 272px"},gap:1.5,alignItems:"start"}}>

            {/* ── NEEDS ATTENTION ── */}
            <Box sx={{bgcolor:"background.paper",borderRadius:"10px",border:`1px solid ${border}`,overflow:"hidden"}}>
              <Box sx={cardHeaderSx}>
                ...
                <Box sx={{display:"flex",alignItems:"center",gap:0.75}}>
                  {overdueCount>0&&(
                    <Box sx={{px:1,py:0.15,borderRadius:"5px",backgroundColor:isDark?RED_15:RED_L}}>
                      <Typography sx={{fontFamily:dm,fontSize:"0.6rem",fontWeight:700,color:isDark?"#ef5350":RED}}>{overdueCount} overdue</Typography>
                    </Box>
                  )}
                  {criticalCount>0&&(
                    <Box sx={{px:1,py:0.15,borderRadius:"5px",backgroundColor:isDark?RED_15:RED_L}}>
                      <Typography sx={{fontFamily:dm,fontSize:"0.6rem",fontWeight:700,color:isDark?"#ef9a9a":"#b71c1c"}}>{criticalCount} critical</Typography>
                    </Box>
                  )}
              ...
              {recentRequests.length===0?(
                <Box sx={{py:6,textAlign:"center"}}>
                  <CheckCircleOutlineOutlinedIcon sx={{fontSize:28,color:isDark?"#333":"#ddd",mb:1}}/>
                  <Typography sx={{fontFamily:dm,fontSize:"0.82rem",color:"text.disabled"}}>All clear — nothing needs attention.</Typography>
                </Box>
              ):(
                recentRequests.map((r,idx)=>{
                  const urg=urgCfg[r.urgency];
                  const st =stCfg[r.status]||{bg:isDark?"#252525":"#f5f5f5",color:"#888"};
                  return (
                    <Box
                      key={r.id}
                      ...
                      sx={{
                        ...
                        backgroundColor:
                          r.urgency==="overdue" ?(isDark?"rgba(198,40,40,0.05)":"rgba(255,235,238,0.4)"):
                          r.urgency==="critical"?(isDark?"rgba(198,40,40,0.03)":"rgba(255,235,238,0.2)"):
                          r.urgency==="soon"    ?(isDark?"rgba(245,197,43,0.02)":"rgba(245,197,43,0.03)"):
                          "transparent",
                        cursor:"pointer",transition:"background 0.12s",
                        "&:hover":{backgroundColor:isDark?"rgba(255,255,255,0.02)":SURF_L},
                      }}
                    >
                      <Box sx={{flex:1,minWidth:0}}>
                        <Box sx={{display:"flex",alignItems:"center",gap:0.75,mb:0.3}}>
                          ...
                          <Typography sx={{fontFamily:dm,fontWeight:600,fontSize:"0.83rem",color:"text.primary",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                            {r.title}
                          </Typography>
                        </Box>
                        <Box sx={{display:"flex",flexWrap:"wrap",gap:1.25,pl:"13px"}}>
                          {r.entity?.name&&(
                            <Typography sx={{fontFamily:dm,fontSize:"0.69rem",color:"text.disabled"}}>{r.entity.name}</Typography>
                          )}
                          {r.event_date&&(
                            <Box sx={{display:"flex",alignItems:"center",gap:0.3}}>
                              <CalendarTodayOutlinedIcon sx={{fontSize:10,color:urg.dot}}/>
                              <Typography sx={{fontFamily:dm,fontSize:"0.69rem",color:urg.color,fontWeight:r.urgency!=="upcoming"?600:400}}>
                          ...
                          {r.venue&&!isMobile&&(
                            <Box sx={{display:"flex",alignItems:"center",gap:0.3}}>
                              <LocationOnOutlinedIcon sx={{fontSize:10,color:"text.disabled"}}/>
                              <Typography sx={{fontFamily:dm,fontSize:"0.69rem",color:"text.disabled"}}>{r.venue}</Typography>
                            </Box>
                          )}
                        </Box>
                      </Box>
            ...
            <Box sx={{display:"flex",flexDirection:"column",gap:1.5}}>
              ...
              <Box sx={{bgcolor:"background.paper",borderRadius:"10px",border:`1px solid ${border}`,overflow:"hidden"}}>
                ...
                <Box sx={{px:1.5,py:1,display:"flex",flexDirection:"column",gap:0.2}}>
                  {/* Phase 1 label */}
                  <Box sx={{display:"flex",alignItems:"center",gap:1,mb:0.25}}>
                    <Divider sx={{flex:1,borderColor:border}}/>
                    <Typography sx={{fontFamily:dm,fontSize:"0.54rem",fontWeight:700,color:"text.disabled",textTransform:"uppercase",letterSpacing:"0.08em",whiteSpace:"nowrap"}}>Phase 1</Typography>
                    <Divider sx={{flex:1,borderColor:border}}/>
                  </Box>
                  {[...PIPELINE_STAGES,{key:"Declined",label:"Declined",icon:CancelOutlinedIcon,navTab:"Declined",isDivider:true}].map((stage)=>{
                    ...
                    return (
                      <React.Fragment key={stage.key}>
                        {isRed&&<Divider sx={{my:0.5,borderColor:border}}/>}
                        {isP2&&stage.key==="On Going"&&(
                          <Box sx={{display:"flex",alignItems:"center",gap:1,my:0.5}}>
                            <Divider sx={{flex:1,borderColor:border}}/>
                            <Typography sx={{fontFamily:dm,fontSize:"0.54rem",fontWeight:700,color:"text.disabled",textTransform:"uppercase",letterSpacing:"0.08em",whiteSpace:"nowrap"}}>Phase 2</Typography>
                            <Divider sx={{flex:1,borderColor:border}}/>
                          </Box>
                        )}
                        <Box
                          ...
                          sx={{
                            display:"flex",alignItems:"center",gap:1.25,py:0.7,px:0.75,borderRadius:"8px",
                            opacity:isP2?0.5:1,
                            ...(isActive?{cursor:"pointer",transition:"background 0.12s","&:hover":{backgroundColor:isDark?"rgba(255,255,255,0.03)":SURF_L}}:{}),
                          }}
                        >
                          <Box sx={{
                            width:24,height:24,borderRadius:"6px",flexShrink:0,
                            display:"flex",alignItems:"center",justifyContent:"center",
                            backgroundColor:isActive?(isRed?(isDark?RED_15:RED_L):isP2?"rgba(59,130,246,0.12)":GOLD_12):(isDark?"#1e1e1e":"#f5f5f5"),
                            color:isActive?(isRed?RED:isP2?"#3b82f6":GOLD):(isDark?"#3a3a3a":"#ccc"),
                          }}>
                            <Icon sx={{fontSize:12}}/>
                          </Box>
                          <Box sx={{flex:1,minWidth:0}}>
              ...
              <Box
                ...
                sx={{
                  ...
                  "&:hover":{borderColor:GOLD,boxShadow:`0 2px 12px ${GOLD_12}`},
                }}
              >
                <Box sx={cardHeaderSx}>
                  <Box sx={{display:"flex",alignItems:"center",gap:1}}>
                ...
                <Box sx={{px:2,py:1.75}}>
                  {!activeSemester?(
                    <Typography sx={{fontFamily:dm,fontSize:"0.78rem",color:"text.disabled"}}>No active semester.</Typography>
                  ):(
                    <>
                      <Box sx={{display:"flex",justifyContent:"space-between",alignItems:"center",mb:1}}>
                        <Box sx={{display:"flex",alignItems:"center",gap:0.5}}>
                          <GroupOutlinedIcon sx={{fontSize:12,color:"text.secondary"}}/>
                          <Typography sx={{fontFamily:dm,fontSize:"0.73rem",color:"text.secondary"}}>Duty days set</Typography>
                        </Box>
                        <Typography sx={{fontFamily:dm,fontSize:"0.92rem",fontWeight:800,color:schedDone?GOLD:RED,letterSpacing:"-0.02em"}}>
                          {scheduleStats.set}
          ...
          <Box sx={{bgcolor:"background.paper",borderRadius:"10px",border:`1px solid ${border}`,overflow:"hidden"}}>
            ...
            <Box sx={{display:"grid",gridTemplateColumns:{xs:"1fr",sm:"repeat(3,1fr)"}}}>
              {sectionWorkload.map((s,idx)=>{
                ...
                const c={
                  News:           {dot:GOLD,     bar:GOLD,     label:"N"},
                  Photojournalism:{dot:"#888",    bar:"#888",   label:"P"},
                  Videojournalism:{dot:"#c49b00", bar:"#c49b00",label:"V"},
                }[s.section]||{dot:GOLD,bar:GOLD,label:"?"};
                return (
                  <Box key={s.section} sx={{
                    px:2.5,py:2,
                    borderRight: {xs:"none",sm:idx<sectionWorkload.length-1?`1px solid ${border}`:"none"},
                    borderBottom:{xs:idx<sectionWorkload.length-1?`1px solid ${border}`:"none",sm:"none"},
                  }}>
                    <Box sx={{display:"flex",alignItems:"center",justifyContent:"space-between",mb:1.25}}>
                      <Box sx={{display:"flex",alignItems:"center",gap:0.75}}>
                        <Box sx={{width:5,height:5,borderRadius:"50%",backgroundColor:c.dot}}/>
                        <Typography sx={{fontFamily:dm,fontSize:"0.8rem",fontWeight:700,color:"text.primary"}}>{s.section}</Typography>
                      </Box>
                      <Typography sx={{fontFamily:dm,fontSize:"0.67rem",color:"text.disabled"}}>
                        {total>0?`${pct.toFixed(0)}% done`:"No assignments"}
                      </Typography>
                    </Box>
                    ...
                    <Box sx={{display:"flex",justifyContent:"space-between"}}>
                      {[
                        {label:"Completed",value:s.completed,color:c.dot},
                        {label:"Pending",  value:s.pending,  color:isDark?"#3a3a3a":"#ccc"},
                      ].map((item)=>(
                        <Box key={item.label} sx={{display:"flex",alignItems:"center",gap:0.5}}>
                          <Box sx={{width:4,height:4,borderRadius:"50%",backgroundColor:item.color}}/>
                          <Typography sx={{fontFamily:dm,fontSize:"0.67rem",color:"text.disabled"}}>{item.label}</Typography>
                          <Typography sx={{fontFamily:dm,fontSize:"0.67rem",fontWeight:700,color:"text.secondary",ml:0.25}}>{item.value}</Typography>
                        </Box>
                      ))}
                    </Box>
                  </Box>
