...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadSemesters() {
      const { data } = await supabase
        .from("semesters").select("id,name,start_date,end_date,is_active,scheduling_open").order("start_date", { ascending: false });
      setSemesters(data || []);
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const td = (requests || []).filter((r) => r.approved_at && r.submitted_at).map((r) => (new Date(r.approved_at) - new Date(r.submitted_at)) / 86400000);
  ...
  return (
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 2.5 }, backgroundColor: "background.default", minHeight: "100%", fontFamily: dm }}>
      ...
      {currentUser?.full_name && (
        <Box sx={{ mb: 2 }}>
          <Typography sx={{ fontFamily: dm, fontWeight: 700, fontSize: { xs: "0.95rem", md: "1rem" }, color: "text.primary", letterSpacing: "-0.02em" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 2, md: 3.5 }, flexWrap: "wrap" }}>
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, mb: 0.5 }}>
                <Box sx={{ width: 5, height: 5, borderRadius: "50%", backgroundColor: GOLD, boxShadow: `0 0 7px ${GOLD}`, animation: "blink 2s ease-in-out infinite", "@keyframes blink": { "0%,100%": { opacity: 1 }, "50%": { opacity: 0.25 } } }} />
                <Typography sx={{ fontFamily: dm, fontSize: "0.6rem", fontWeight: 700, color: GOLD, textTransform: "uppercase", letterSpacing: "0.13em" }}>Live</Typography>
              </Box>
              {forApprovalCount > 0 ? (
            ...
            <Box sx={{ display: "flex", gap: { xs: 2, md: 3 }, flexWrap: "wrap" }}>
              {[
                ...
                <Box key={s.label}>
                  <Typography sx={{ fontFamily: dm, fontSize: "1.25rem", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1, color: s.value > 0 ? s.activeColor : "rgba(255,255,255,0.15)" }}>{s.value}</Typography>
                  <Typography sx={{ fontFamily: dm, fontSize: "0.6rem", color: "rgba(255,255,255,0.28)", mt: 0.2, textTransform: "uppercase", letterSpacing: "0.07em" }}>{s.label}</Typography>
                </Box>
              ))}
            </Box>
          </Box>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
            ...
            {kpiCards.map((k) => {
              ...
              return (
                ...
                >
                  ...
                  <Box>
                    <Typography sx={{ fontFamily: dm, fontSize: "1.55rem", fontWeight: 800, lineHeight: 1, letterSpacing: "-0.03em", color: k.isRed ? RED : "text.primary" }}>{k.value}</Typography>
                    <Typography sx={{ fontFamily: dm, fontSize: "0.67rem", color: "text.disabled", mt: 0.2 }}>{k.label} · {k.sub}</Typography>
                  </Box>
                </Box>
              );
            })}
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 272px" }, gap: 1.5, alignItems: "start" }}>
            ...
            <Box sx={{ bgcolor: "background.paper", borderRadius: "10px", border: `1px solid ${border}`, overflow: "hidden" }}>
              ...
              {recentRequests.length === 0 ? (
                ...
                recentRequests.map((r, idx) => {
                  ...
                  return (
                    <Box key={r.id} onClick={() => goToRequest(r.id, r.status)}
                      sx={{ px: 2.5, py: 1.4, borderBottom: idx < recentRequests.length - 1 ? `1px solid ${border}` : "none", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, borderLeft: "3px solid", borderLeftColor: urg.border, backgroundColor: r.urgency === "overdue" ? (isDark ? "rgba(198,40,40,0.05)" : "rgba(255,235,238,0.4)") : r.urgency === "critical" ? (isDark ? "rgba(198,40,40,0.03)" : "rgba(255,235,238,0.2)") : r.urgency === "soon" ? (isDark ? "rgba(245,197,43,0.02)" : "rgba(245,197,43,0.03)") : "transparent", cursor: "pointer", transition: "background 0.12s", "&:hover": { backgroundColor: isDark ? "rgba(255,255,255,0.02)" : SURF_L } }}
                    >
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, mb: 0.3 }}>
                          ...
                          <Typography sx={{ fontFamily: dm, fontWeight: 600, fontSize: "0.83rem", color: "text.primary", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.title}</Typography>
                        </Box>
                        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.25, pl: "13px" }}>
                          {r.entity?.name && <Typography sx={{ fontFamily: dm, fontSize: "0.69rem", color: "text.disabled" }}>{r.entity.name}</Typography>}
                          {r.event_date && (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
              ...
              {/* Scheduling */}
              <Box onClick={() => navigate("/admin/duty-schedule-view")}
                sx={{ bgcolor: "background.paper", borderRadius: "10px", border: `1px solid ${border}`, overflow: "hidden", cursor: "pointer", transition: "border-color 0.15s,box-shadow 0.15s", "&:hover": { borderColor: GOLD, boxShadow: `0 2px 12px ${GOLD_12}` } }}
              >
                <Box sx={cardHeaderSx}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <Box sx={cardAccent} />
          ...
          <Box sx={{ bgcolor: "background.paper", borderRadius: "10px", border: `1px solid ${border}`, overflow: "hidden" }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(3,1fr)" } }}>
              {sectionWorkload.map((s, idx) => {
                ...
                const c = { News: { dot: GOLD, bar: GOLD }, Photojournalism: { dot: "#888", bar: "#888" }, Videojournalism: { dot: "#c49b00", bar: "#c49b00" } }[s.section] || { dot: GOLD, bar: GOLD };
                return (
                  <Box key={s.section} sx={{ px: 2.5, py: 2, borderRight: { xs: "none", sm: idx < sectionWorkload.length - 1 ? `1px solid ${border}` : "none" }, borderBottom: { xs: idx < sectionWorkload.length - 1 ? `1px solid ${border}` : "none", sm: "none" } }}>
