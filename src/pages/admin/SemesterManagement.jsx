...
const EMPTY_FORM = { name: "", start_date: "", end_date: "", is_active: false, scheduling_open: false };
...
export default function SemesterManagement() {
  ...
  const loadSemesters = useCallback(async () => {
    ...
    const { data, error: fetchErr } = await supabase
      .from("semesters").select("*").order("created_at", { ascending: false });
  ...
  const openCreate = () => { setEditTarget(null); setForm(EMPTY_FORM); setSaveError(""); setDialogOpen(true); };
  const openEdit   = (row) => {
    setEditTarget(row);
    setForm({ name: row.name, start_date: row.start_date, end_date: row.end_date, is_active: row.is_active, scheduling_open: row.scheduling_open });
    setSaveError(""); setDialogOpen(true);
  };

  const handleSave = async () => {
    if (!form.name.trim() || !form.start_date || !form.end_date) { setSaveError("Please fill in all fields."); return; }
    if (form.start_date >= form.end_date) { setSaveError("End date must be after start date."); return; }
    setSaving(true); setSaveError("");
    try {
      if (form.is_active) await supabase.from("semesters").update({ is_active: false }).neq("id", editTarget?.id ?? "");
      if (editTarget) {
        const { error: e } = await supabase.from("semesters").update(form).eq("id", editTarget.id);
        if (e) throw e;
      } else {
        const { error: e } = await supabase.from("semesters").insert(form);
        if (e) throw e;
      }
      setDialogOpen(false); loadSemesters();
  ...
  const toggleActive = async (row) => {
    if (!row.is_active) await supabase.from("semesters").update({ is_active: false }).neq("id", row.id);
  ...
  const columns = [
    {
      ...
      renderCell: (p) => (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, height: "100%" }}>
          ...
          {p.row.is_active && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, px: 1, py: 0.25, borderRadius: "5px", backgroundColor: GOLD_08 }}>
              <Box sx={{ width: 4, height: 4, borderRadius: "50%", backgroundColor: GOLD }} />
              <Typography sx={{ fontFamily: dm, fontSize: "0.62rem", fontWeight: 700, color: "#b45309" }}>Active</Typography>
            </Box>
          )}
        </Box>
      ),
    ...
    {
      field: "actions", headerName: "", flex: 1.8, minWidth: 260, sortable: false,
        ...
        <Box sx={{
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, px: 1.25, py: 0.35, borderRadius: "6px", backgroundColor: activeSemester.scheduling_open ? "#eff6ff" : isDark ? "rgba(255,255,255,0.04)" : "rgba(53,53,53,0.04)" }}>
            {activeSemester.scheduling_open
              ? <LockOpenOutlinedIcon sx={{ fontSize: 11, color: "#1d4ed8" }} />
              : <LockOutlinedIcon     sx={{ fontSize: 11, color: "text.secondary" }} />}
            <Typography sx={{ fontFamily: dm, fontSize: "0.68rem", fontWeight: 600, color: activeSemester.scheduling_open ? "#1d4ed8" : "text.secondary" }}>
              {activeSemester.scheduling_open ? "Scheduling Open" : "Scheduling Closed"}
            </Typography>
          </Box>
        </Box>
      )}
