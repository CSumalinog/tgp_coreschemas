...
const TAB_DESCRIPTIONS = {
  ...
  "For Approval":
    "Staffers have been assigned. Review and give final approval.",
  Approved:
    "Requests that have been fully approved and are ready for coverage.",
  "On Going":
    "Coverage currently in progress — staff have timed in and are on-site.",
  Completed:
    "Coverage finished — full time in, time out, and duration records.",
...
export default function AdminRequestManagement() {
    ...
    if (selectedSem !== "all") {
      ...
      if (sem) {
        ...
        filtered = filtered.filter((r) => {
          ...
          const dateField = ["On Going", "Completed"].includes(r.status)
            ? r.event_date
            : r.submitted_at;
  ...
  const getBaseSource = (key) => {
    ...
    if (key === "For Approval")
      return forApproval || requests.filter((r) => r.status === "For Approval");
  ...
  const buildColumns = () => {
    ...
    if (key === "Approved")
      return [titleCol, clientCol, eventDateCol, approvedByCol, actionCol];
    if (key === "On Going")
      return [
        titleCol,
        clientCol,
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 3 }}>
        ...
        >
          Manage all coverage requests — review, forward to sections, and
          approve or decline.
        </Typography>
      </Box>
      ...
      >
        ...
        <Box
          ref={filterRef}
          sx={{ position: "relative", flexShrink: 0, pb: "1px" }}
        >
      ...
      <Box sx={{ mt: 2, mb: 2 }}>
        <Typography
          sx={{ fontFamily: dm, fontSize: "0.75rem", color: "text.secondary" }}
        >
