...
function getHolidayTagStyle(title, isDark) {
  ...
  const swatch =
    HOLIDAY_TAG_PALETTE[Math.abs(hash) % HOLIDAY_TAG_PALETTE.length];
...
function getPhilippineHolidays(year) {
  ...
  Object.entries(fixedHolidays).forEach(([d, t]) => {
    holidays[`${year}-${d}`] = t;
  });
  Object.entries(specialDays).forEach(([d, t]) => {
    holidays[`${year}-${d}`] = t;
  });
  ...
  const fetchMonthData = async (firstDay, lastDay) => {
    try {
      ...
      const resolvedDefault =
        settingsRow?.default_slots ?? FALLBACK_DEFAULT_SLOTS;
      ...
      >
        ...
        >
          ...
          {calendarDays.map((date, idx) => {
            ...
            const isDisabled =
              !date.isCurrentMonth || isPast || remainingSlots <= 0 || blocked;
            ...
            if (isWeekend && date.isCurrentMonth)
              cellBg = isDark ? "#1a1a1a" : "rgba(53,53,53,0.02)";
            if (!date.isCurrentMonth)
              cellBg = isDark ? "#141414" : "rgba(53,53,53,0.015)";
            if (blocked && date.isCurrentMonth)
              cellBg = isDark ? "#1a0a0a" : "#fff5f5";

            const tooltipTitle = !date.isCurrentMonth
              ? ""
              : blocked
                ? "Blocked by Admin"
                : isPast
                ...
                >
                  <Box
                    ...
                    sx={{
                      ...
                      ...(isToday && { color: GOLD }),
                      ...(holidayTitle &&
                        !isToday && { color: "#15803d", fontWeight: 700 }),
                      ...(blocked &&
                        date.isCurrentMonth &&
                        !isToday && { color: "#d32f2f", fontWeight: 700 }),
                    }}
                  >
                    {date.dayNumber}
                  </Box>
