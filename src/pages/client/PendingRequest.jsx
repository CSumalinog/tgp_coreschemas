...
import {
  ...
  Typography, Chip, Stack, IconButton, CircularProgress,
} from "@mui/material";
      ...
      <Box sx={{ height: 500, width: "100%", bgcolor: "white", borderRadius: 2, boxShadow: 1 }}>
        {loading ? (
          ...
          <DataGrid
            ...
            sx={{ "& .MuiDataGrid-cell": { outline: "none" }, "& .MuiDataGrid-columnHeaders": { backgroundColor: "#f5f5f5" } }}
          />
        )}
      </Box>

      <Dialog open={!!selectedRequest} onClose={handleClose} fullWidth
        PaperProps={{ sx: { fontFamily: "'Helvetica Neue', sans-serif", borderRadius: 3, width: 600, height: "70vh" } }}
      >
        <IconButton onClick={handleClose} sx={{ position: "absolute", right: 8, top: 8, color: "#757575" }}>
          <CloseIcon />
        </IconButton>
        ...
        <DialogContent dividers sx={{ "& *": { fontSize: "0.9rem", fontFamily: "'Helvetica Neue', sans-serif" } }}>
          {selectedRequest && (
            <Box sx={{ display: "grid", gridTemplateColumns: "1.2fr 2.8fr", gap: 0.1, alignItems: "center" }}>
              ...
              <Typography variant="body2">{selectedRequest.eventTitle}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Description:</Typography>
              <Typography variant="body2">{selectedRequest.details.description}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Requested Date:</Typography>
              <Typography variant="body2">{selectedRequest.details.requestedDate}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Requested Time:</Typography>
              <Typography variant="body2">{selectedRequest.details.timeRange}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Location:</Typography>
              <Typography variant="body2">{selectedRequest.details.venue}</Typography>
              ...
              <Typography variant="body2">{selectedRequest.details.client}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Contact Person:</Typography>
              <Typography variant="body2">{selectedRequest.details.contactPerson}</Typography>

              <Typography variant="subtitle2" sx={{ color: "#9e9e9e" }}>Contact Info:</Typography>
              <Typography variant="body2">{selectedRequest.details.contactInfo}</Typography>
