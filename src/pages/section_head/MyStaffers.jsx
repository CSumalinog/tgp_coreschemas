...
function CountBadge({ value, colorDot, colorText, colorBg, isDark }) {
  ...
  return (
    <Box sx={{ display: "inline-flex", alignItems: "center", gap: 0.55, px: 1.1, py: 0.3, borderRadius: "6px", backgroundColor: isZero ? (isDark ? "rgba(255,255,255,0.04)" : "#f5f5f5") : (isDark ? `${colorDot}18` : colorBg) }}>
...
export default function MyStaffers() {
  ...
  useEffect(() => {
    async function loadSemester() {
      const { data } = await supabase
        .from("semesters").select("id, name").eq("is_active", true).single();
  ...
  const loadStaffers = useCallback(async () => {
    ...
    const merged = profiles
      .map((p) => ({ ...p, dutyDay: dutyMap[p.id] ?? null, total: totalMap[p.id] || 0, completed: completedMap[p.id] || 0, pending: pendingMap[p.id] || 0 }))
      ...
      renderCell: (params) => (
        <Box sx={{ display: "flex", alignItems: "center", height: "100%" }}>
          <Box sx={{ display: "inline-flex", alignItems: "center", gap: 0.55, px: 1.1, py: 0.3, borderRadius: "6px", backgroundColor: isDark ? "rgba(168,85,247,0.1)" : "#f5f3ff" }}>
            <Typography sx={{ fontFamily: dm, fontSize: "0.66rem", fontWeight: 700, color: isDark ? "#a855f7" : "#7c3aed", letterSpacing: "0.04em" }}>{params.value}</Typography>
          </Box>
        </Box>
      ),
    },
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: "background.default", minHeight: "100%", fontFamily: dm }}>
      ...
      {!activeSemester && <Alert severity="info" sx={{ mb: 2, borderRadius: "8px", fontFamily: dm, fontSize: "0.78rem" }}>No active semester. Duty day info won't be available.</Alert>}
      ...
      <Box sx={{ width: "100%", overflowX: "auto" }}>
        <Box sx={{ minWidth: 600, bgcolor: "background.paper", borderRadius: "10px", border: `1px solid ${border}`, overflow: "hidden", height: 500 }}>
          {loading ? (
            ...
            <Box sx={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Typography sx={{ fontFamily: dm, fontSize: "0.82rem", color: "text.secondary" }}>No staffers found in your division.</Typography>
            </Box>
          ) : (
            <DataGrid
              rows={rows}
