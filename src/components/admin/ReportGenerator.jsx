// src/pages/admin/ReportGenerator.jsx
...
export default function ReportGenerator({ selectedSemester, isAllTime }) {
  ...
  async function fetchReportData() {
    ...
    try {
      // 1. Coverage requests (full fields)
      ...
      // 2. Assignments — join request info for full staffer event detail
      ...
      // ── Section workload ──
      const sectionMap = {
        News:            { pending: 0, completed: 0, total: 0 },
        Photojournalism: { pending: 0, completed: 0, total: 0 },
        Videojournalism: { pending: 0, completed: 0, total: 0 },
      };
      ...
      // ── Full staffer workload with event details ──
      // stafferMap[id] = { name, section, count, events: [{title, eventDate, venue, client, status}] }
      const stafferMap = {};
      (assignments || []).forEach((a) => {
        if (!a.assigned_to) return;
        const name = a.profiles?.full_name || a.assigned_to;
        if (!stafferMap[a.assigned_to]) {
          stafferMap[a.assigned_to] = { name, section: a.section, count: 0, events: [] };
        }
        stafferMap[a.assigned_to].count++;
        stafferMap[a.assigned_to].events.push({
          title:     a.coverage_requests?.title     || "—",
          eventDate: fmtDate(a.coverage_requests?.event_date),
          venue:     a.coverage_requests?.venue     || "—",
          client:    a.coverage_requests?.entity?.name || "—",
          status:    a.status,
        });
      });
      // Sort events within each staffer by event date
      Object.values(stafferMap).forEach((s) => {
        s.events.sort((a, b) => new Date(a.eventDate) - new Date(b.eventDate));
      });
      // Sort staffers by assignment count desc, then name
      const stafferList = Object.values(stafferMap)
      ...
      // ── Events with no staffer assigned (Forwarded but no assignment row) ──
      const assignedRequestIds = new Set((assignments || []).map((a) => a.coverage_requests?.title));
      const unassignedEvents = (requests || [])
        .filter((r) => r.status === "Forwarded" && !assignedRequestIds.has(r.title))
        .map((r) => ({
          title:     r.title,
          client:    r.entity?.name || "—",
          eventDate: fmtDate(r.event_date),
  ...
  // ── Generate & download PDF (A4 portrait) ────────────────────────────────────
  function generatePDF() {
    ...
    try {
      ...
      // ── Reusable autoTable wrapper that updates y ──
      const addTable = (opts) => {
        autoTable(doc, {
          ...opts,
          startY: y,
          styles:             { fontSize: 8, cellPadding: 2.5, font: "helvetica", overflow: "linebreak" },
          headStyles:         { fillColor: [245, 197, 43], textColor: [33, 33, 33], fontStyle: "bold", fontSize: 8 },
          alternateRowStyles: { fillColor: [249, 249, 249] },
          margin:             { left: MARGIN, right: MARGIN },
          tableWidth:         W - MARGIN * 2,
          didDrawPage: (data) => {
            // Redraw footer on every new page autoTable creates
            drawFooter(doc, W);
          },
        });
      ...
      // ── 1. Summary Stats (2-column card grid) ──
      ...
      addTable({
        head: [["Section", "Total Assignments", "Pending", "Completed"]],
        body: preview.sectionWorkload.map((s) => [s.section, s.total, s.pending, s.completed]),
        columnStyles: { 0: { cellWidth: 70 }, 1: { cellWidth: 50 }, 2: { cellWidth: 30 }, 3: { cellWidth: 32 } },
      });

      // ── 3. Coverage per Client ──
      sectionHeading("Coverage by Client / Entity");
      addTable({
        head: [["Client", "Total", "Approved", "Declined", "Pending"]],
        body: preview.clientList.map((c) => [c.name, c.total, c.approved, c.declined, c.pending]),
        columnStyles: { 0: { cellWidth: 80 } },
      });

      // ════════════════════════════════════════════════════════
      // STAFFER WORKLOAD — one sub-section per staffer
      // ════════════════════════════════════════════════════════
      if (preview.stafferList.length > 0) {
        ...
        preview.stafferList.forEach((staffer) => {
          ...
          doc.setFontSize(9);
          doc.setTextColor(33, 33, 33);
          doc.text(staffer.name, MARGIN + 4, y + 7);
          doc.setFont("helvetica", "normal");
          doc.setFontSize(8);
          doc.setTextColor(100, 100, 100);
          doc.text(
            `${staffer.section}  ·  ${staffer.count} assignment${staffer.count !== 1 ? "s" : ""}`,
          ...
          if (staffer.events.length === 0) {
            ...
            doc.text("No events found.", MARGIN + 4, y);
            y += 8;
          } else {
            addTable({
              head: [["Event Title", "Client", "Event Date", "Venue", "Status"]],
              body: staffer.events.map((e) => [e.title, e.client, e.eventDate, e.venue, e.status]),
              columnStyles: {
                0: { cellWidth: 55 },
                1: { cellWidth: 35 },
                2: { cellWidth: 25 },
      ...
      if (preview.approvedRequests.length > 0) {
        ...
        sectionHeading("Approved Coverage Requests");
        addTable({
          head: [["Event Title", "Client", "Event Date", "Venue", "Submitted"]],
          body: preview.approvedRequests.map((r) => [r.title, r.client, r.eventDate, r.venue, r.submittedAt]),
          columnStyles: {
            0: { cellWidth: 55 },
            1: { cellWidth: 35 },
            2: { cellWidth: 25 },
      ...
      if (preview.declinedRequests.length > 0) {
        y = addPageIfNeeded(doc, y, 20);
        sectionHeading("Declined Requests");
        addTable({
          head: [["Event Title", "Client", "Event Date", "Submitted"]],
          body: preview.declinedRequests.map((r) => [r.title, r.client, r.eventDate, r.submittedAt]),
          columnStyles: { 0: { cellWidth: 70 }, 1: { cellWidth: 50 }, 2: { cellWidth: 30 }, 3: { cellWidth: 32 } },
        });
      }
      ...
      if (preview.pendingRequests.length > 0) {
        y = addPageIfNeeded(doc, y, 20);
        sectionHeading("Pending / Unresolved Requests");
        addTable({
          head: [["Event Title", "Client", "Event Date", "Status", "Submitted"]],
          body: preview.pendingRequests.map((r) => [r.title, r.client, r.eventDate, r.status, r.submittedAt]),
          columnStyles: { 0: { cellWidth: 55 }, 1: { cellWidth: 35 }, 2: { cellWidth: 25 }, 3: { cellWidth: 28 }, 4: { cellWidth: 25 } },
        });
      }

      // ════════════════════════════════════════════════════════
      // EVENTS WITH NO STAFFER ASSIGNED
      // ════════════════════════════════════════════════════════
      if (preview.unassignedEvents.length > 0) {
        y = addPageIfNeeded(doc, y, 20);
        sectionHeading("Forwarded Events with No Staffer Assigned");
        addTable({
          head: [["Event Title", "Client", "Event Date", "Venue"]],
          body: preview.unassignedEvents.map((r) => [r.title, r.client, r.eventDate, r.venue]),
          columnStyles: { 0: { cellWidth: 70 }, 1: { cellWidth: 45 }, 2: { cellWidth: 30 }, 3: { cellWidth: 37 } },
        });
      }
      ...
      const filename = isAllTime
        ...
        : `coverage-report-${(selectedSemester?.name || "report").replace(/\s+/g, "-").toLowerCase()}.pdf`;
      doc.save(filename);
    } catch (err) {
  ...
  return (
    <Box sx={{ mt: 0 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
        <Box>
          <Typography sx={{ fontWeight: 700, fontSize: "1rem", color: "#212121" }}>Report Generation</Typography>
          <Typography sx={{ fontSize: "0.78rem", color: "#9e9e9e", mt: 0.3 }}>
        ...
        >
          {fetching
            ? <><CircularProgress size={12} sx={{ color: "#212121", mr: 0.8 }} />Loading…</>
            : "Preview Report"
          }
        </Button>
      </Box>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 2, borderRadius: 2, fontSize: "0.8rem" }}>{error}</Alert>
      )}

      {/* Empty state */}
      {!preview && !fetching && (
        <Box sx={{ p: 4, textAlign: "center", bgcolor: "white", borderRadius: 2, border: "1px solid #e0e0e0" }}>
          <AssignmentOutlinedIcon sx={{ fontSize: 36, color: "#e0e0e0", mb: 1 }} />
          <Typography sx={{ fontSize: "0.82rem", color: "#bdbdbd" }}>
      ...
      {preview && (
        <Box sx={{ bgcolor: "white", borderRadius: 2, border: "1px solid #e0e0e0", overflow: "hidden" }}>
          ...
          <Box sx={{
            px: 2.5, py: 2,
            backgroundColor: "#fffde7", borderBottom: "1px solid #fff176",
            ...
            <Button
              ...
              startIcon={generating
                ? <CircularProgress size={13} sx={{ color: "#212121" }} />
          ...
          <Box sx={{ px: 2.5, pt: 2, pb: 1.5, display: "flex", gap: 1.5, flexWrap: "wrap" }}>
            {[
              { label: "Total",          value: preview.stats.total,      icon: <AssignmentOutlinedIcon />,         bg: "#f5f5f5", color: "#757575" },
              { label: "Approved",       value: preview.stats.approved,   icon: <CheckCircleOutlineOutlinedIcon />, bg: "#e8f5e9", color: "#2e7d32" },
              { label: "Declined",       value: `${preview.stats.declined} (${preview.stats.declineRate}%)`, icon: <CancelOutlinedIcon />, bg: "#ffebee", color: "#c62828" },
              { label: "Completion",     value: `${preview.stats.completionRate}%`, icon: <TrendingUpOutlinedIcon />, bg: "#e3f2fd", color: "#1565c0" },
              { label: "Avg Turnaround", value: preview.stats.avgTurnaround ? `${preview.stats.avgTurnaround}d` : "—", icon: <AccessTimeOutlinedIcon />, bg: "#fff3e0", color: "#e65100" },
            ].map((s) => (
              <Box key={s.label} sx={{ flex: "1 1 100px", bgcolor: "#f9f9f9", borderRadius: 2, border: "1px solid #f0f0f0", p: 1.5 }}>
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 0.6 }}>
                  <Typography sx={{ fontSize: "0.7rem", color: "#9e9e9e" }}>{s.label}</Typography>
                  <Box sx={{ p: 0.5, borderRadius: 1, backgroundColor: s.bg, color: s.color, display: "flex" }}>
                    {React.cloneElement(s.icon, { sx: { fontSize: 12 } })}
                  </Box>
                </Box>
                <Typography sx={{ fontSize: "1.2rem", fontWeight: 700, color: "#212121", lineHeight: 1 }}>{s.value}</Typography>
              </Box>
            ))}
          </Box>
          ...
          <Box sx={{ px: 2.5, py: 1.5 }}>
            <Typography sx={{ fontSize: "0.72rem", fontWeight: 600, color: "#9e9e9e", textTransform: "uppercase", letterSpacing: 0.5, mb: 1 }}>
              Section Workload
            </Typography>
            <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
              {preview.sectionWorkload.map((s) => (
                <Box key={s.section} sx={{ flex: "1 1 120px", bgcolor: "#f9f9f9", borderRadius: 2, border: "1px solid #f0f0f0", p: 1.5 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.8, mb: 0.5 }}>
                    <Box sx={{ width: 7, height: 7, borderRadius: "50%", backgroundColor: SECTION_COLORS[s.section]?.color }} />
                    <Typography sx={{ fontSize: "0.75rem", fontWeight: 600, color: "#212121" }}>{s.section}</Typography>
                  </Box>
                  <Typography sx={{ fontSize: "1rem", fontWeight: 700, color: "#212121" }}>{s.total}</Typography>
                  <Typography sx={{ fontSize: "0.68rem", color: "#9e9e9e" }}>{s.pending} pending · {s.completed} done</Typography>
                </Box>
              ))}
            </Box>
          </Box>
          ...
          {/* Staffer workload preview (top 5, full detail in PDF) */}
          <Box sx={{ px: 2.5, py: 1.5 }}>
            <Typography sx={{ fontSize: "0.72rem", fontWeight: 600, color: "#9e9e9e", textTransform: "uppercase", letterSpacing: 0.5, mb: 1 }}>
              Staffer Workload
            </Typography>
            {preview.stafferList.length === 0 ? (
              <Typography sx={{ fontSize: "0.78rem", color: "#bdbdbd" }}>No assignments found.</Typography>
            ) : (
              <Box sx={{ display: "flex", flexDirection: "column", gap: 0.6 }}>
                {preview.stafferList.slice(0, 5).map((s, i) => (
                  <Box key={s.name} sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 1.5, py: 0.8, bgcolor: "#f9f9f9", borderRadius: 2, border: "1px solid #f0f0f0" }}>
                    <Typography sx={{ fontSize: "0.72rem", fontWeight: 700, color: "#bdbdbd", width: 14 }}>#{i + 1}</Typography>
                    <Typography sx={{ fontSize: "0.82rem", fontWeight: 500, flex: 1 }}>{s.name}</Typography>
                    <Chip
                      label={s.section}
                      size="small"
                      sx={{ fontSize: "0.65rem", height: 18, backgroundColor: SECTION_COLORS[s.section]?.bg, color: SECTION_COLORS[s.section]?.color }}
                    />
                    <Typography sx={{ fontSize: "0.78rem", fontWeight: 700, color: "#212121", minWidth: 60, textAlign: "right" }}>
                      {s.count} {s.count === 1 ? "assign." : "assigns."}
                    </Typography>
                  </Box>
                ))}
                {preview.stafferList.length > 5 && (
                  <Typography sx={{ fontSize: "0.72rem", color: "#bdbdbd", pl: 0.5 }}>
          ...
          {/* PDF contents summary footer */}
          <Box sx={{ px: 2.5, py: 1.2, backgroundColor: "#f9f9f9", borderTop: "1px solid #f0f0f0" }}>
