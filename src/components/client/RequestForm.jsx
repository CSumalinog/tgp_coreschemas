...
function DateChipInput({
  ...
  const allFilled =
    eventDays.length > 0 && eventDays.every((d) => d.fromTime && d.toTime);
    ...
    <Box ref={wrapRef}>
      ...
      >
        <CalendarTodayOutlinedIcon
          sx={{
            fontSize: 15,
            color: calOpen ? "#f5c52b" : "text.disabled",
            flexShrink: 0,
          }}
        />
        <Typography
          sx={{
            fontSize: "0.83rem",
            color: eventDays.length > 0 ? "text.primary" : "text.disabled",
            flex: 1,
          }}
        >
          {eventDays.length === 0
        ...
        {eventDays.length > 0 && (
          <Box
            ...
            onClick={(e) => {
              e.stopPropagation();
              eventDays.forEach((d) => onToggleDay(d.date));
            }}
            sx={{
              fontSize: "0.7rem",
              color: "text.disabled",
      ...
      {error && (
        <Typography
          sx={{ fontSize: "0.72rem", color: "#ef4444", mt: 0.25, ml: 0.5 }}
        >
          {error}
        </Typography>
      )}
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              <Typography sx={{ fontSize: "0.68rem", color: "text.disabled" }}>
                {selectedKeys.length}/{MAX_DAY_SELECTION} selected · grayed days
                already added
              </Typography>
            </Box>
            <Button
      ...
      {eventDays.length > 0 && (
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <Box sx={{ mt: calOpen ? 0.5 : 1 }}>
            {isMultiDay && (
              <Box
                sx={{ display: "flex", justifyContent: "flex-end", mb: 0.75 }}
              >
                <Button
                  size="small"
                  variant="outlined"
                    ...
                    <Box
                      ...
                      sx={{
                        ...
                        "&:hover": {
                          borderColor: "#ef4444",
                          color: "#ef4444",
                          backgroundColor: isDark ? "#1f0000" : "#fef2f2",
                        },
                      }}
                    >
                      ×
...
export default function CoverageRequestDialog({
  ...
  const handleToggleDay = useCallback((key) => {
    setEventDays((prev) => {
      ...
      return [...prev, { date: key, fromTime: null, toTime: null }].sort(
  ...
  const handleTimeChange = useCallback((key, field, val) => {
    setEventDays((prev) =>
      prev.map((d) => (d.date === key ? { ...d, [field]: val } : d)),
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (existingRequest) {
      ...
      if (existingRequest.event_days?.length > 0) {
        setEventDays(
          existingRequest.event_days.map((d) => ({
            date: d.date,
            fromTime: d.from_time
              ? new Date(`1970-01-01T${d.from_time}`)
              : null,
        ...
        setEventDays([
          {
            ...
            fromTime: existingRequest.from_time
              ? new Date(`1970-01-01T${existingRequest.from_time}`)
              : null,
            toTime: existingRequest.to_time
      ...
      const isExistingMultiDay = !!(
        existingRequest.is_multiday && existingRequest.event_days?.length > 1
      );
      const rawServices =
      ...
      setServices(
        isExistingMultiDay
          ? Object.fromEntries(
              Object.entries(rawServices).map(([k, v]) => [
                k,
                Math.round(v / existingDays),
              ]),
            )
          : rawServices,
      );
  ...
  const validate = () => {
    ...
    if (eventDays.length === 0) {
      newErrors.eventDays = "Please select at least one date.";
      hasError = true;
    } else if (eventDays.some((d) => !d.fromTime || !d.toTime)) {
      newErrors.eventDays =
        "Please set From and To times for all selected days.";
      hasError = true;
    }
    if (!venue) {
  ...
  const submitForm = async (isDraft = false) => {
    ...
    setLoading(true);
    try {
      const sorted = [...eventDays].sort((a, b) =>
      ...
      const savedServices = isMD
        ? Object.fromEntries(
            Object.entries(services).map(([k, v]) => [k, v * sorted.length]),
          )
        : services;
      ...
      >
        ...
        >
          ...
          <FormSection label="Event Details">
            ...
            <Box sx={{ mt: 1.5 }}>
              <Typography
                sx={{ fontSize: "0.78rem", color: "text.secondary", mb: 0.75 }}
              >
          ...
          <FormSection label="Services Needed">
            ...
            >
              ...
              <Box component="span" sx={{ color: "text.disabled" }}>
                Max {MAX_PAX} per service{isMultiDay ? " per day" : ""}.
              </Box>
            </Typography>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 0.75 }}>
              {SERVICES.map((service) => {
                ...
                const totalSlots = isMultiDay
                  ? services[service] * eventDays.length
                  : services[service];
                return (
                  <Box key={service}>
                    <Box
            ...
            {isMultiDay &&
              Object.values(services).reduce((s, v) => s + v, 0) > 0 && (
                <Box
                  sx={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  ...
                  <Typography sx={{ fontSize: "0.71rem", color: "#15803d" }}>
                    {Object.values(services).reduce((s, v) => s + v, 0) *
                      eventDays.length}{" "}
                    total staff slots across {eventDays.length} days
                  </Typography>
                </Box>
              )}
        ...
        <DialogContent sx={{ pt: 2 }}>
          {selectedDaysSummary && (
            ...
            >
              ...
              >
                {selectedDaysSummary}
                {isMultiDay ? ` — ${eventDays.length} coverage slots` : ""}
              </Typography>
            </Box>
          )}
          <Typography
          ...
          {submitError && (
            <Alert
              severity="error"
              sx={{ mt: 2, borderRadius: "10px", fontSize: "0.82rem" }}
            >
              {submitError}
            </Alert>
          )}
