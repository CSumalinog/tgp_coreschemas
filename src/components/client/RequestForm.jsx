...
export default function CoverageRequestDialog({
  ...
  const submitForm = async (isDraft = false) => {
    ...
    if (!isDraft) {
      ...
      if (!title) {
        setErrors((prev) => ({ ...prev, title: "Please enter at least an event title to save as draft." }));
        return;
      }
    }
    ...
    <>
        ...
        >
          ...
          <FormSection label="Event Details">
            ...
            <LocalizationProvider dateAdapter={AdapterDateFns}>
              <Box sx={{ display: "flex", gap: 1, mt: 0.5 }}>
                <Box sx={{ flex: 5, minWidth: 0 }}>
                  <DatePicker
                    label="Event Date" value={date} disabled={loading}
                    onChange={(val) => { setDate(val); if (val) setErrors((p) => ({ ...p, date: "" })); }}
                    slotProps={{ textField: { fullWidth: true, margin: "dense", error: !!errors.date, helperText: errors.date, sx: errorFieldSx(!!errors.date), FormHelperTextProps: { sx: helperSx } } }}
                  />
                </Box>
                <Box sx={{ flex: 3, minWidth: 0 }}>
                  <TimePicker
                    label="From" value={fromTime} disabled={loading}
                    onChange={(val) => { setFromTime(val); if (val) setErrors((p) => ({ ...p, fromTime: "" })); }}
                    slotProps={{ textField: { fullWidth: true, margin: "dense", error: !!errors.fromTime, helperText: errors.fromTime, sx: errorFieldSx(!!errors.fromTime), FormHelperTextProps: { sx: helperSx } } }}
                  />
                </Box>
                <Box sx={{ flex: 3, minWidth: 0 }}>
                  <TimePicker
                    label="To" value={toTime} disabled={loading}
                    onChange={(val) => { setToTime(val); if (val) setErrors((p) => ({ ...p, toTime: "" })); }}
                    slotProps={{ textField: { fullWidth: true, margin: "dense", error: !!errors.toTime, helperText: errors.toTime, sx: errorFieldSx(!!errors.toTime), FormHelperTextProps: { sx: helperSx } } }}
                  />
                </Box>
              </Box>
            ...
            <TextField
              ...
              sx={errorFieldSx(!!errors.venue)} FormHelperTextProps={{ sx: helperSx }}
            />
          </FormSection>
                  ...
                  <Box
                    ...
                    onClick={() => {
                      ...
                      setServices((prev) => {
                        ...
                        if (Object.values(updated).reduce((s, v) => s + v, 0) > 0)
          ...
          <FormSection label="Client Information">
            ...
            <Box sx={{ display: "flex", gap: 1, mt: 0.5 }}>
              ...
              <FormControl fullWidth margin="dense" required error={!!errors.clientType} sx={fieldSx}>
                <InputLabel sx={{ fontSize: "0.85rem", ...(errors.clientType && { color: "#ef4444" }) }}>
                  Client Type
                </InputLabel>
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value) setErrors((p) => ({ ...p, clientType: "" }));
                  }}
                  sx={{
              ...
              <FormControl
                fullWidth margin="dense" required error={!!errors.entity}
            ...
            {isOthers && (
              <TextField
                label="Specify Entity Name" fullWidth margin="dense" value={otherEntity}
      ...
      >
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          {submitError && (
            <Alert severity="error" sx={{ mt: 2, borderRadius: 1.5, fontSize: "0.82rem" }}>
              {submitError}
            </Alert>
          )}
          {loading && (
        ...
        <Box sx={{
          ...
          <Button
            onClick={() => submitForm(true)} disabled={loading} size="small"
            sx={{ textTransform: "none", fontSize: "0.82rem", color: "text.secondary" }}
          >
            Save as Draft
          </Button>
