  ...
  return (
    <Box
      ...
      sx={{
        ...
        gap: 0.6, px: 2, py: 0.75, borderRadius: "8px",
...
function GPSStatusPill({ status, isDark }) {
  const cfg = {
    idle: {
      icon: GpsNotFixedIcon, color: "text.secondary",
      bg: isDark ? "rgba(255,255,255,0.04)" : "rgba(53,53,53,0.04)",
      label: "Checking location…",
    },
    checking: {
      icon: GpsNotFixedIcon, color: "#3b82f6",
      bg: "rgba(59,130,246,0.08)", label: "Getting your location…",
    },
    verified: {
      icon: GpsFixedIcon, color: "#15803d",
      bg: "rgba(34,197,94,0.08)", label: "On campus ✓",
    },
    outside: {
      icon: GpsOffIcon, color: "#b45309",
      bg: "rgba(245,197,43,0.08)", label: "Location unverified",
    },
    denied: {
      icon: GpsOffIcon, color: "#b45309",
      bg: "rgba(245,197,43,0.08)", label: "Location access denied",
    },
    unavailable: {
      icon: GpsOffIcon, color: "text.secondary",
      bg: isDark ? "rgba(255,255,255,0.04)" : "rgba(53,53,53,0.04)",
...
function SelfieCapture({ selfieFile, onCapture, onClear, isDark, border, isEmergency }) {
  ...
  const stampImage = useCallback((file) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (!blob) return;
        const stampedFile = new File([blob], `login-proof-${Date.now()}.jpg`, { type: "image/jpeg" });
      ...
      {!selfieFile ? (
        <Box
          ...
          sx={{
            ...
            "&:hover": { borderColor: isEmergency ? "#c62828" : GOLD, backgroundColor: isEmergency ? "rgba(198,40,40,0.06)" : GOLD_08 },
          }}
        >
          <Box sx={{ width: 44, height: 44, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: isEmergency ? "rgba(198,40,40,0.1)" : isDark ? "rgba(245,197,43,0.1)" : "rgba(245,197,43,0.12)" }}>
        ...
        <Box sx={{ position: "relative", borderRadius: "10px", overflow: "hidden", border: `1px solid rgba(34,197,94,0.35)` }}>
          <Box component="img" src={previewUrl} alt="Selfie preview" sx={{ width: "100%", maxHeight: 220, objectFit: "cover", display: "block" }} />
...
export function TimeInModal({ open, assignment, isDark, submitting, error, onClose, onConfirm }) {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: 3, py: 2, borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center" }}>
      ...
      PaperProps={{
        sx: {
          ...
          "@keyframes alertPulse": {
            "0%, 100%": { boxShadow: "0 8px 48px rgba(239,68,68,0.35)", borderColor: "#ef4444" },
            "50%":      { boxShadow: "0 8px 64px rgba(239,68,68,0.65)", borderColor: "#dc2626" },
          },
        },
      }}
    >
      <Box sx={{ height: 4, backgroundColor: "#ef4444", borderRadius: "14px 14px 0 0", animation: "barFlash 1.5s ease-in-out infinite", "@keyframes barFlash": { "0%, 100%": { opacity: 1 }, "50%": { opacity: 0.5 } } }} />

      {/* ── Header — no close button ── */}
      <Box sx={{ px: 3, py: 2, borderBottom: `1px solid rgba(239,68,68,0.2)`, display: "flex", alignItems: "flex-start", backgroundColor: "rgba(239,68,68,0.04)" }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
          <Box sx={{ position: "relative", width: 10, height: 10, flexShrink: 0 }}>
            <Box sx={{ position: "absolute", inset: 0, borderRadius: "50%", bgcolor: "#ef4444", animation: "ping 1.4s ease-in-out infinite", "@keyframes ping": { "0%": { transform: "scale(1)", opacity: 0.7 }, "100%": { transform: "scale(2.2)", opacity: 0 } } }} />
            <Box sx={{ position: "absolute", inset: 0, borderRadius: "50%", bgcolor: "#ef4444" }} />
          </Box>
          <Box>
            <Typography sx={{ fontFamily: dm, fontWeight: 700, fontSize: "0.9rem", color: "#dc2626" }}>
              {isEmergency ? "⚠ Emergency Assignment" : "⚠ Coverage Starting Soon"}
            </Typography>
            <Typography sx={{ fontFamily: dm, fontSize: "0.7rem", color: "text.secondary" }}>
              {isEmergency ? "You've been urgently reassigned" : "You must time in now"}
            </Typography>
          </Box>
        </Box>
      </Box>
      ...
      <Box sx={{ px: 3, py: 2.5, display: "flex", flexDirection: "column", gap: 1.5 }}>
        <Box sx={{ px: 1.75, py: 1.5, borderRadius: "8px", border: `1px solid rgba(239,68,68,0.25)`, backgroundColor: "rgba(239,68,68,0.05)", display: "flex", flexDirection: "column", gap: 0.9 }}>
          <Typography sx={{ fontFamily: dm, fontSize: "0.88rem", fontWeight: 700, color: "text.primary", lineHeight: 1.3 }}>
            {req?.title}
          </Typography>
          {req?.from_time && (
            <MetaItem icon={<AccessTimeOutlinedIcon sx={{ fontSize: 11, color: "#ef4444" }} />}>
              {req.from_time}{req.to_time ? ` — ${req.to_time}` : ""}
            </MetaItem>
          )}
          {req?.venue && (
            <MetaItem icon={<LocationOnOutlinedIcon sx={{ fontSize: 11, color: "#ef4444" }} />}>
              {req.venue}
            </MetaItem>
          )}
        </Box>

        <Typography sx={{ fontFamily: dm, fontSize: "0.78rem", color: "#dc2626", lineHeight: 1.6, px: 0.25, fontWeight: 600 }}>
          {isEmergency
      ...
      <Box sx={{ px: 3, py: 1.75, borderTop: `1px solid rgba(239,68,68,0.2)`, display: "flex", justifyContent: "flex-end", backgroundColor: "rgba(239,68,68,0.03)" }}>
        <Box
          onClick={() => { onClose(); onTimeIn(assignment); }}
          sx={{ display: "flex", alignItems: "center", gap: 0.6, px: 1.75, py: 0.65, borderRadius: "8px", cursor: "pointer", backgroundColor: "#ef4444", color: "#fff", fontFamily: dm, fontSize: "0.8rem", fontWeight: 600, transition: "background-color 0.15s", "&:hover": { backgroundColor: "#dc2626" } }}
        >
