...
if (typeof document !== "undefined" && !document.getElementById("dash-fonts")) {
  ...
  l.id   = "dash-fonts";
  l.rel  = "stylesheet";
  l.href = "https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700;0,9..40,800;1,9..40,400&display=swap";
...
const HOVER_BG       = "rgba(245,197,43,0.10)";
...
function HelpPopover({ open, onClose, anchorRef }) {
  ...
  if (!open) return null;
  return (
    <Box ref={ref} sx={{ position: "absolute", bottom: "calc(100% + 8px)", left: 12, width: 256, backgroundColor: WHITE, border: "1px solid rgba(53,53,53,0.10)", borderRadius: "12px", zIndex: 1400, boxShadow: "0 -4px 24px rgba(53,53,53,0.15), 0 8px 32px rgba(53,53,53,0.10)", overflow: "hidden", animation: "helpPopup 0.16s cubic-bezier(0.34,1.4,0.64,1)", "@keyframes helpPopup": { from: { opacity: 0, transform: "translateY(6px)" }, to: { opacity: 1, transform: "translateY(0)" } } }}>
      <Box sx={{ px: 2, pt: 1.75, pb: 1.25, borderBottom: "1px solid rgba(53,53,53,0.08)", display: "flex", alignItems: "center", gap: 1 }}>
        <HelpOutlineIcon sx={{ fontSize: 15, color: GOLD, flexShrink: 0 }} />
        <Box>
          <Typography sx={{ fontFamily: dm, fontSize: "0.82rem", fontWeight: 700, color: CHARCOAL, lineHeight: 1.3 }}>How to request coverage</Typography>
          <Typography sx={{ fontFamily: dm, fontSize: "0.68rem", color: "rgba(53,53,53,0.55)", mt: 0.2 }}>Quick guide to submitting a request</Typography>
        </Box>
      </Box>
      <Box sx={{ px: 2, py: 1.75, display: "flex", flexDirection: "column", gap: 1.25 }}>
        {[
          ...
          <Box key={i} sx={{ display: "flex", alignItems: "flex-start", gap: 1.25 }}>
            <Box sx={{ width: 20, height: 20, borderRadius: "50%", backgroundColor: step.note ? "rgba(53,53,53,0.08)" : CHARCOAL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, mt: 0.1 }}>
              <Typography sx={{ fontFamily: dm, fontSize: "0.62rem", fontWeight: 700, color: step.note ? CHARCOAL : WHITE, lineHeight: 1 }}>{step.note ? "!" : i + 1}</Typography>
            </Box>
            <Typography sx={{ fontFamily: dm, fontSize: "0.78rem", color: "rgba(53,53,53,0.8)", lineHeight: 1.55 }}>{step.text}</Typography>
          </Box>
        ))}
      </Box>
      <Box sx={{ px: 2, py: 1.25, borderTop: "1px solid rgba(53,53,53,0.08)", display: "flex", alignItems: "center", gap: 0.75, backgroundColor: "rgba(53,53,53,0.02)" }}>
        <TrackChangesIcon sx={{ fontSize: 12, color: "rgba(53,53,53,0.4)", flexShrink: 0 }} />
        <Typography sx={{ fontFamily: dm, fontSize: "0.73rem", color: "rgba(53,53,53,0.6)" }}>Track your requests in Request Tracker</Typography>
      </Box>
    </Box>
  );
}
...
function ProfileDropdown({ open, currentUser, userProfile, onClose, footerRef }) {
  ...
  const row = (onClick, children, danger = false) => (
    <Box onClick={onClick} sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 2, py: 1, cursor: "pointer", transition: "background 0.12s", "&:hover": { backgroundColor: danger ? "rgba(198,40,40,0.05)" : "rgba(53,53,53,0.04)" } }}>
      {children}
    </Box>
  );
  return (
    <Box ref={ref} sx={{ position: "absolute", bottom: "100%", left: 0, right: 0, backgroundColor: WHITE, border: "1px solid rgba(53,53,53,0.12)", borderBottom: "none", borderRadius: "12px 12px 0 0", zIndex: 1400, boxShadow: "0 -8px 24px rgba(53,53,53,0.12)", animation: "dropup 0.18s cubic-bezier(0.34,1.4,0.64,1)", "@keyframes dropup": { from: { opacity: 0, transform: "translateY(6px)" }, to: { opacity: 1, transform: "translateY(0)" } } }}>
      <Box sx={{ px: 2, py: 1.25, borderBottom: "1px solid rgba(53,53,53,0.08)" }}>
        <Typography sx={{ fontFamily: dm, fontSize: "0.72rem", color: "rgba(53,53,53,0.45)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentUser?.email || ""}</Typography>
      </Box>
      ...
      {row(toggleDark, <>
        ...
        <Switch checked={isDark} onChange={toggleDark} onClick={(e) => e.stopPropagation()} size="small" sx={{ "& .MuiSwitch-switchBase.Mui-checked": { color: CHARCOAL }, "& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track": { backgroundColor: CHARCOAL }, "& .MuiSwitch-track": { backgroundColor: "rgba(53,53,53,0.2)" } }} />
      </>)}
...
function SidebarContent({ currentUser, userProfile, avatarUrl, onClose, isMobile }) {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: dm, position: "relative", backgroundColor: SIDEBAR_BG }}>
      ...
      {/* Logo */}
      <Box sx={{ px: 2.5, height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, borderBottom: `1px solid ${SIDEBAR_BORDER}` }}>
        ...
        {isMobile && (
          <IconButton onClick={onClose} size="small" sx={{ color: TEXT_SECONDARY, "&:hover": { color: TEXT_PRIMARY, backgroundColor: HOVER_BG } }}>
      ...
      {/* Nav */}
      <Box sx={{ flex: 1, overflowY: "auto", px: 1.5, py: 1.5, "&::-webkit-scrollbar": { width: 0 } }}>
      ...
      {/* Help row */}
      <Box ref={helpRef} sx={{ position: "relative", borderTop: `1px solid ${SIDEBAR_BORDER}`, px: 2, py: 1, display: "flex", alignItems: "center" }}>
        <HelpPopover open={helpOpen} onClose={() => setHelpOpen(false)} anchorRef={helpRef} />
        <Box onClick={() => { setDropdownOpen(false); setHelpOpen((p) => !p); }}
          sx={{ display: "inline-flex", alignItems: "center", gap: 0.75, px: 1.25, py: 0.5, borderRadius: "20px", cursor: "pointer", border: `1px solid ${helpOpen ? "rgba(245,197,43,0.5)" : "rgba(245,197,43,0.25)"}`, backgroundColor: helpOpen ? "rgba(245,197,43,0.1)" : "rgba(245,197,43,0.06)", transition: "all 0.15s", "&:hover": { borderColor: "rgba(245,197,43,0.45)", backgroundColor: "rgba(245,197,43,0.1)" } }}
        >
          <HelpOutlineIcon sx={{ fontSize: 13, color: TEXT_SECONDARY }} />
          <Typography sx={{ fontFamily: dm, fontSize: "0.75rem", fontWeight: 500, color: TEXT_SECONDARY, lineHeight: 1 }}>Help</Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ position: "relative" }}>
        ...
        <Box ref={footerRef} onClick={() => { setHelpOpen(false); setDropdownOpen((p) => !p); }}
          sx={{ px: 2, py: 1.5, borderTop: `1px solid ${SIDEBAR_BORDER}`, display: "flex", alignItems: "center", gap: 1.25, cursor: "pointer", userSelect: "none", transition: "background 0.15s", backgroundColor: dropdownOpen ? HOVER_BG : "transparent", "&:hover": { backgroundColor: HOVER_BG } }}
        >
...
function CalendarLegendSection() {
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Typography sx={{ fontFamily: dm, fontSize: "0.6rem", fontWeight: 700, color: TEXT_LABEL, letterSpacing: "0.1em", textTransform: "uppercase", px: 1.25, mb: 0.75 }}>CALENDAR</Typography>
...
function CollapsibleLegend({ title, items }) {
  ...
  return (
    <Box sx={{ mx: 1, mb: 1, border: `1px solid ${SIDEBAR_BORDER}`, borderRadius: "10px", overflow: "hidden" }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 1.5, py: 1, cursor: "pointer", "&:hover": { backgroundColor: HOVER_BG } }} onClick={() => setOpen((p) => !p)}>
        ...
        <Box sx={{ px: 1.5, pb: 1.5, display: "flex", flexDirection: "column", gap: 0.85 }}>
          {items.map((item, idx) => (
            <Box key={idx} sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box sx={{ width: 9, height: 9, borderRadius: "50%", flexShrink: 0, backgroundColor: item.isBorder ? "transparent" : item.color, border: item.isBorder ? "2px solid rgba(53,53,53,0.3)" : "none" }} />
  ...
  return (
    <NavLink to={to} style={{ textDecoration: "none" }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, px: 1.25, py: 0.8, borderRadius: "9px", cursor: "pointer", position: "relative", mb: 0.2, backgroundColor: active ? ACTIVE_BG : "transparent", transition: "background 0.15s", "&:hover": { backgroundColor: active ? ACTIVE_BG : HOVER_BG }, "&::before": active ? { content: '""', position: "absolute", left: 0, top: "20%", height: "60%", width: "2.5px", borderRadius: "0 2px 2px 0", backgroundColor: GOLD } : {} }}>
        ...
        <Typography sx={{ fontFamily: dm, fontSize: "0.81rem", fontWeight: active ? 600 : 400, color: active ? GOLD : TEXT_SECONDARY, flex: 1, transition: "color 0.15s", lineHeight: 1 }}>
          {label}
        </Typography>
      </Box>
    </NavLink>
...
function ClientLayout() {
  ...
  const sidebarNode = (
    <SidebarContent
      currentUser={currentUser} userProfile={userProfile} avatarUrl={avatarUrl}
      onClose={() => setMobileOpen(false)} isMobile={isMobile}
    />
  );

  return (
    <Box sx={{ display: "flex", height: "100vh", fontFamily: dm, backgroundColor: "#F5F5F7" }}>
      {!isMobile && (
        <Box sx={{ width: SIDEBAR_W, flexShrink: 0, display: "flex", flexDirection: "column", height: "100vh", zIndex: 1200, borderRight: "1px solid rgba(53,53,53,0.12)" }}>
          {sidebarNode}
        </Box>
      )}
      {isMobile && (
        <Drawer open={mobileOpen} onClose={() => setMobileOpen(false)} variant="temporary" ModalProps={{ keepMounted: true }}
          sx={{ "& .MuiDrawer-paper": { width: SIDEBAR_W, border: "none", borderRight: "1px solid rgba(53,53,53,0.12)" } }}
        >
          {sidebarNode}
        </Drawer>
      )}

      <Box sx={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", backgroundColor: "#F5F5F7", minWidth: 0 }}>
        <Box sx={{ display: "flex", alignItems: "center", px: 3, height: 60, flexShrink: 0, backgroundColor: WHITE, borderBottom: `1px solid ${BORDER}`, gap: 2 }}>
          {isMobile && (
            <IconButton onClick={() => setMobileOpen(true)} size="small" sx={{ color: "rgba(53,53,53,0.6)", borderRadius: "8px", p: 0.75, mr: 0.5, "&:hover": { backgroundColor: "rgba(53,53,53,0.04)", color: CHARCOAL } }}>
              <MenuIcon sx={{ fontSize: 20 }} />
            </IconButton>
          )}
          <Button variant="contained" size="small" startIcon={<AddIcon sx={{ fontSize: 15 }} />} onClick={() => setOpenDialog(true)}
      ...
      <CoverageRequestDialog open={openDialog} handleClose={() => setOpenDialog(false)} onSuccess={() => setOpenDialog(false)} />
    </Box>
  );
}
