...
import {
  ...
  Box, Typography, Button, Avatar, Switch,
} from "@mui/material";
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: dm, position: "relative", backgroundColor: SIDEBAR_BG }}>
      ...
      <Box sx={{ flex: 1, overflowY: "auto", px: 1.5, pb: 2, "&::-webkit-scrollbar": { width: 0 } }}>
        <Typography sx={{ fontFamily: dm, fontSize: "0.6rem", fontWeight: 700, color: TEXT_LABEL, letterSpacing: "0.1em", textTransform: "uppercase", px: 1.25, mb: 0.5 }}>MENU</Typography>
        {NAV_ITEMS.map((item) => (
        ...
        {showLegend && (
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ mx: 1, mb: 1, border: `1px solid ${BORDER}`, borderRadius: "10px", overflow: "hidden" }}>
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 1.5, py: 1, cursor: "pointer", "&:hover": { backgroundColor: HOVER_BG } }} onClick={() => setShowInstructions((p) => !p)}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <InfoOutlinedIcon sx={{ fontSize: 14, color: GOLD }} />
                  <Typography sx={{ fontFamily: dm, fontSize: "0.78rem", color: TEXT_SECONDARY }}>How to Request</Typography>
                </Box>
                <KeyboardArrowDownIcon sx={{ fontSize: 15, color: TEXT_SECONDARY, transition: "transform 0.2s", transform: showInstructions ? "rotate(180deg)" : "rotate(0deg)" }} />
              </Box>
                ...
                <Box sx={{ px: 1.5, pb: 1.5, display: "flex", flexDirection: "column", gap: 0.75 }}>
                  {[
                    ...
                    <Box key={i} sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
                      <Typography sx={{ fontFamily: dm, fontSize: "0.68rem", color: GOLD, fontWeight: 700, flexShrink: 0, mt: 0.1 }}>{i + 1}.</Typography>
                      <Typography sx={{ fontFamily: dm, fontSize: "0.72rem", color: TEXT_SECONDARY, lineHeight: 1.5 }}>{text}</Typography>
                    </Box>
                  ))}
                </Box>
              </Collapse>
...
function ClientLayout() {
  ...
  const sidebarNode = (
    <SidebarContent currentUser={currentUser} userProfile={userProfile} avatarUrl={avatarUrl} onClose={() => setMobileOpen(false)} isMobile={isMobile} onCreateRequest={() => setOpenDialog(true)} />
  );
