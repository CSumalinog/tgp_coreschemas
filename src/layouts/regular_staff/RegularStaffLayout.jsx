...
import {
  ...
  Drawer, useMediaQuery, Badge, Tooltip, Switch, useTheme,
} from "@mui/material";
...
// ─── Brand tokens (shared with AdminLayout & SectionHeadLayout) ───────────────
const GOLD        = "#F5C52B";
...
function ProfileDropdown({ open, currentUser, userProfile, onClose, footerRef }) {
  ...
  const row = (onClick, children, danger = false) => (
    <Box
      ...
      sx={{
        ...
        "&:hover": {
          backgroundColor: danger ? "rgba(198,40,40,0.05)" : HOVER_BG,
        },
      }}
    >
      {children}
  ...
  return (
    ...
    >
      ...
      {/* Profile & Settings */}
      {row(() => { navigate("/staff/profile"); onClose(); }, <>
