...
// topbar / content uses light border
...
const MENU_SECTIONS = [
  {
    ...
    items: [
      ...
      { label: "Request Management", to: "request-management",  Icon: DescriptionOutlinedIcon },
      {
        label: "Scheduling",
        Icon: SchoolOutlinedIcon,
        children: [
          { label: "Semester Management", to: "semester-management", Icon: CalendarMonthOutlinedIcon },
          { label: "Duty Schedule View",  to: "duty-schedule-view",  Icon: TableChartOutlinedIcon   },
        ],
      },
  ...
  return (
    <Box ref={ref} sx={{ position: "absolute", bottom: "100%", left: 0, right: 0, backgroundColor: WHITE, border: "1px solid rgba(53,53,53,0.12)", borderBottom: "none", borderRadius: "12px 12px 0 0", zIndex: 1400, boxShadow: "0 -8px 24px rgba(0,0,0,0.3)", animation: "dropup 0.18s cubic-bezier(0.34,1.4,0.64,1)", "@keyframes dropup": { from: { opacity: 0, transform: "translateY(6px)" }, to: { opacity: 1, transform: "translateY(0)" } } }}>
      ...
      {row(toggleDark, <>
        ...
        <Switch checked={isDark} onChange={toggleDark} onClick={(e) => e.stopPropagation()} size="small"
          sx={{ "& .MuiSwitch-switchBase.Mui-checked": { color: GOLD }, "& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track": { backgroundColor: GOLD }, "& .MuiSwitch-track": { backgroundColor: "rgba(53,53,53,0.2)" } }}
        />
      </>)}
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: dm, position: "relative", backgroundColor: SIDEBAR_BG }}>
        ...
        {MENU_SECTIONS.map((section) => (
          <Box key={section.group} sx={{ mb: 2 }}>
            ...
            {section.items.map((item) => {
                ...
                return (
                  <Box key={item.label}>
                    <NavItem
                      ...
                      trailing={
                        <KeyboardArrowDownIcon sx={{ fontSize: 15, color: TEXT_ICON, transition: "transform 0.22s", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)" }} />
                      }
                    />
              ...
              return <NavItem key={item.to} label={item.label} Icon={item.Icon} to={item.to} />;
            })}
          </Box>
        ))}
      </Box>
      ...
      <Box sx={{ position: "relative" }}>
        ...
        <Box ref={footerRef} onClick={() => setDropdownOpen((p) => !p)}
...
function NavItem({ label, Icon, to, onClick, isActive, isChild, trailing }) {
    ...
    <Box onClick={onClick} sx={{ display: "flex", alignItems: "center", gap: 1.25, px: 1.25, py: 0.8, borderRadius: "9px", cursor: "pointer", position: "relative", mb: 0.2, backgroundColor: active ? ACTIVE_BG : "transparent", transition: "background 0.15s", "&:hover": { backgroundColor: active ? ACTIVE_BG : HOVER_BG }, "&::before": active ? { content: '""', position: "absolute", left: 0, top: "20%", height: "60%", width: "2.5px", borderRadius: "0 2px 2px 0", backgroundColor: GOLD } : {} }}>
      {Icon && (
        <Box sx={{ width: 24, height: 24, borderRadius: "7px", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: active ? ACTIVE_ICON_BG : "transparent", flexShrink: 0, transition: "background 0.15s" }}>
          <Icon sx={{ fontSize: isChild ? 14 : 15, color: active ? GOLD : TEXT_ICON, transition: "color 0.15s" }} />
        </Box>
      )}
  ...
  if (to) return <NavLink to={to} style={{ textDecoration: "none" }}>{inner}</NavLink>;
  return inner;
}
...
function AdminLayout() {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      const { data } = await supabase
        .from("profiles").select("full_name, role, section, division, position, avatar_url")
  ...
  const sidebarNode = (
    <SidebarContent
      ...
      avatarUrl={avatarUrl} onClose={() => setMobileOpen(false)}
      isMobile={isMobile}
    />
  );

  return (
    <Box sx={{ display: "flex", height: "100vh", fontFamily: dm, backgroundColor: isDark ? "#0D0D0F" : "#F5F5F7" }}>
      {!isMobile && (
        <Box sx={{ width: SIDEBAR_W, flexShrink: 0, display: "flex", flexDirection: "column", height: "100vh", zIndex: 1200, borderRight: `1px solid ${SIDEBAR_BORDER}` }}>
          {sidebarNode}
        </Box>
      )}
      {isMobile && (
        <Drawer open={mobileOpen} onClose={() => setMobileOpen(false)} variant="temporary" ModalProps={{ keepMounted: true }}
          sx={{ "& .MuiDrawer-paper": { width: SIDEBAR_W, border: "none", borderRight: `1px solid ${SIDEBAR_BORDER}` } }}
        >
          {sidebarNode}
        </Drawer>
      )}
        ...
        <Box sx={{ display: "flex", alignItems: "center", px: 3, height: 60, flexShrink: 0, backgroundColor: WHITE, borderBottom: `1px solid ${BORDER}`, gap: 2 }}>
          {isMobile && (
            <IconButton onClick={() => setMobileOpen(true)} size="small"
              sx={{ color: "rgba(53,53,53,0.6)", borderRadius: "8px", p: 0.75, mr: 0.5, "&:hover": { backgroundColor: "rgba(53,53,53,0.04)", color: CHARCOAL } }}
            >
